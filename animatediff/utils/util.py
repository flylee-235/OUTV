...
def save_videos_grid(videos: torch.Tensor, path: str, rescale=False, n_rows=6, fps=8, n_frames=None):
    ...
    if path.lower().endswith(".mp4"):
        frames = np.stack(outputs, axis=0)
...
def replace_video(video, videos_masked, pad, smooth=True):
    ...
    if smooth:
        ...
        if cv2 is not None:
            ...
            mask_img = PIL.Image.fromarray((mask[:, :, 0] * 255).astype(np.uint8))
            mask_img = mask_img.filter(PIL.ImageFilter.GaussianBlur(radius=max(sigma_h, sigma_w)))
