...
def get_prompt(frame, lmm_tokenizer, lmm_model):
    path = 'infer/temp.jpg'
    frame = np.uint8((frame.numpy().transpose(1,2,0)+1)/2*255)
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    for video_name in video_paths[start_index:]:
        ...
        prompts = fixed_prompt if fixed_prompt != '' else get_prompt(pixel_values_valid[0, video_length//2,:,:,:], lmm_tokenizer, lmm_model)
