...
def main(
    ...
    use_precomputed_sam3_dense: bool=False,
):
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    if use_ip_plus_cross_attention:
        if image_encoder_name == 'SAM':
            if use_precomputed_sam3_dense:
                zero_rank_print("use_precomputed_sam3_dense=True; skip SAM ViT-B runtime encoder")
            else:
    ...
    if dataset_name == 'single-video-sam3-tta':
        train_dataset = SingleVideoIntrinsicDataset(**OmegaConf.to_container(train_data))
    else:
                    ...
                    with torch.no_grad():
                        if image_encoder_name == 'SAM':
                            if ip_plus_condition != 'video':
                                assert ValueError
                            if use_precomputed_sam3_dense and 'sam3_dense_embeddings' in batch:
                                image_embeds = batch['sam3_dense_embeddings'].to(local_rank)
                                image_embeds = rearrange(image_embeds, "b f c h w -> b f (h w) c")
                                uncond_image_embeds = torch.zeros_like(image_embeds)
                            # HWC, unit8
                            elif (batch['image_tensors_sam'] != 0).any().item():
